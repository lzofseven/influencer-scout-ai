...
export const MetricsChart: React.FC<MetricsChartProps> = ({ data }) => {
  const [metric, setMetric] = useState<'followers' | 'engagement'>('followers');

  const parseValue = (valueStr: string, type: 'followers' | 'engagement'): number => {
    if (!valueStr) return 0;
    const cleanStr = valueStr.toUpperCase().replace(/,/g, '.').replace(/%/g, '').trim();
    let multiplier = 1;
  ...
  const chartData = useMemo(() => {
    return data
      .map(inf => ({
        ...inf,
        rawValue: parseValue(metric === 'followers' ? inf.followerCount : inf.engagementRate, metric)
      }))
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div className="flex bg-gray-50 p-1 rounded-md border border-gray-100">
           <button 
             ...
             className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-all rounded ${metric === 'followers' ? 'bg-black text-white shadow-sm' : 'text-gray-400 hover:text-black'}`}
           >
             Seguidores
           </button>
           <button 
             onClick={() => setMetric('engagement')}
             className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-all rounded ${metric === 'engagement' ? 'bg-black text-white shadow-sm' : 'text-gray-400 hover:text-black'}`}
           >
             Engajamento
           </button>
        </div>
